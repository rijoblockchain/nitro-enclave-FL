...
def weight_scalling_factor(client_trn_data):
    ...
    global_count = (6368+6368)*32      #sum([tf.data.experimental.cardinality(clients_trn_data[org1]+clients_trn_data[org2]).numpy() for client_name in client_names])*bs
...
np.save('org1_local_weights.npy', encrypted_weights, allow_pickle=True)
# b = np.load('org1_local_weights.npy', allow_pickle=True)

# Save the encrypted_key to a file
f = open("org1_encrypted_key.txt", "wb")
...
# print(len(org1_local_weights_encrypted))
# print(len(data_arr))
