...
function main {
    ...
    getAdminOrg
    getDomain $ADMINORG
    kubectl exec --stdin --tty  $(kubectl get pods -n $NAMESPACE |grep cli-peer$COUNT-$ADMINORG|awk '{print $1}') --namespace $NAMESPACE -- /bin/bash  ./scripts/sign-org-config.sh $ADMINORG $NEW_ORG
