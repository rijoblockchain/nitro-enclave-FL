        ...
        for ORG in $ORDERER_ORGS; do
          ...
          while [[ "$COUNT" -le $NUM_ORDERERS ]]; do             
            ...
            if [ $COUNT -eq 1 ]; then
                ...
                echo "          ClientTLSCert: $ORDERER_GENERAL_TLS_CERTIFICATE"
                echo "          ServerTLSCert: $ORDERER_GENERAL_TLS_CERTIFICATE"
            else
...
function generateChannelArtifacts() {
  ...
  configtxgen -profile HealthcareOrdererGenesis -channelID system-channel -outputBlock $GENESIS_BLOCK_FILE
  if [ "$?" -ne 0 ]; then
  ...
  for ORG in $PEER_ORGS; do
     ...
     set -x
	configtxgen -profile HealthcareChannel -outputAnchorPeersUpdate $ANCHOR_TX_FILE -channelID $CHANNEL_NAME -asOrg ${ORG}
	res=$?
	{ set +x; } 2>/dev/null
     if [ "$?" -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  # Note: For some unknown reason (at least for now) the block file can't be
  # named orderer.genesis.block or the orderer will fail to launch!
  configtxgen -profile HealthcareApplicationGenesis -outputBlock $GENESIS_BLOCK_FILE -channelID $CHANNEL_NAME
  if [ "$?" -ne 0 ]; then
    fatal "Failed to generate orderer genesis block"
  fi

  log "Generating channel configuration transaction "
  #configtxgen -profile OrgsChannel -outputCreateChannelTx $CHANNEL_TX_FILE -channelID $CHANNEL_NAME
  osnadmin channel join --channel-id $CHANNEL_NAME --config-block $GENESIS_BLOCK_FILE -o $ORDERER_HOST:$ORDERER_PORT --ca-file "$CA_CHAINFILE" #--client-cert "$ORDERER_GENERAL_TLS_CERTIFICATE" --client-key "$ORDERER_GENERAL_TLS_PRIVATEKEY"
  ...
  for ORG in $PEER_ORGS; do
     ...
     # joinChannel ORG
     peer channel join -b $GENESIS_BLOCK_FILE

     log "Generating anchor peer update transaction for $ORG "
    ...
    jq '.channel_group.groups.Application.groups.'${CORE_PEER_LOCALMSPID}'.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "'$PEER_HOST'","port": '$PORT'}]},"version": "0"}}' ${CORE_PEER_LOCALMSPID}config.json > ${CORE_PEER_LOCALMSPID}modified_config.json
     
    # Compute a config update, based on the differences between 
    # {orgmsp}config.json and {orgmsp}modified_config.json, write
    # it as a transaction to {orgmsp}anchors.tx
    createConfigUpdate ${CHANNEL_NAME} ${CORE_PEER_LOCALMSPID}config.json ${CORE_PEER_LOCALMSPID}modified_config.json $ANCHOR_TX_FILE
    
    peer channel update -o $ORDERER_HOST:$ORDERER_PORT --ordererTLSHostnameOverride $ORDERER_HOST -c $CHANNEL_NAME -f $ANCHOR_TX_FILE --tls --cafile "$CA_CHAINFILE"
